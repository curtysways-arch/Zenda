...
function onUploadsFinished() {
    console.log('🔨 Limpiando locks, compilando Next.js en VPS y reiniciando PM2...');
    conn.exec(`cd ${VPS_BASE} && rm -f .next/lock && pkill -f "next build" || true && npm run build && pm2 restart zenda-app`, (err, stream) => {
        if (err) {
        ...
        stream.on('close', (code) => {
            console.log(`\n🚀 ¡Eliminación de negocios en cascada corregida con éxito! (Código final: ${code})`);
            conn.end();
        });
    });
}
