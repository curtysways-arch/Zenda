...
const filesToDeploy = [
    ...
    'src/components/superadmin/NegocioActions.tsx'
];
...
function onUploadsFinished() {
    console.log('🧹 Eliminando .next cache anterior, compilando Next.js desde cero y reiniciando PM2...');
    conn.exec(`cd ${VPS_BASE} && pkill -f "next build" || true && rm -rf .next && npm run build && pm2 restart zenda-app --update-env`, (err, stream) => {
        if (err) {
        ...
        stream.on('close', (code) => {
            console.log(`\n🚀 ¡Compilación desde cero y reinicio en VPS completados! (Código final: ${code})`);
            conn.end();
        });
    });
}
