        ...
        const db = new sqlite3.Database(dbPath, (err) => {
            ...
            db.serialize(() => {
                ...
                db.run(`INSERT OR IGNORE INTO "Staff" ("id", "name", "role", "businessId", "active", "workingHours", "updatedAt") 
                    ...
                    if (err) {
                        db.run(`INSERT OR IGNORE INTO "Staff" ("id", "name", "role", "negocioId", "active", "workingHours", "updatedAt") 
                                VALUES ('staff-id', 'Ana García', 'Estilista', 'demo-id', 1, '{"monday":{"start":"08:00","end":"20:00"}}', CURRENT_TIMESTAMP)`);
                    }
                });

                // Cancha (Service)
                db.run(`INSERT OR IGNORE INTO "Cancha" ("id", "nombre", "precio", "duracion", "negocioId", "updatedAt") 
                        VALUES ('service-id', 'Corte de cabello', 8, 50, 'demo-id', CURRENT_TIMESTAMP)`, (err) => {
                    if (err) {
                        db.run(`INSERT OR IGNORE INTO "Cancha" ("id", "nombre", "precio", "duracion", "businessId", "updatedAt") 
                                VALUES ('service-id', 'Corte de cabello', 8, 50, 'demo-id', CURRENT_TIMESTAMP)`);
                    }
                });
...
async function run() {
    ...
    console.log("✅ Proceso completado en todas las BD.");
}
