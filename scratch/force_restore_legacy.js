...
async function run() {
    console.log(`🛠️ Restaurando Negocio (Legacy Names) en: ${dbPath}`);
    const db = new sqlite3.Database(dbPath, (err) => {
        ...
        db.serialize(() => {
            ...
            // Usar "Category" si existe, o "TipoCancha"
            db.run(`INSERT OR IGNORE INTO "Category" ("id", "nombre", "businessId", "updatedAt") 
                    VALUES ('cat-id', 'Masajes', 'demo-id', CURRENT_TIMESTAMP)`, function(err) {
                if (err && err.message.includes('no such table')) {
                    db.run(`INSERT OR IGNORE INTO "TipoCancha" ("id", "nombre", "businessId", "updatedAt") 
                            VALUES ('cat-id', 'Masajes', 'demo-id', CURRENT_TIMESTAMP)`);
                }
            });

            // Usar "Cancha" para Service
            db.run(`INSERT OR IGNORE INTO "Cancha" ("id", "nombre", "descripcion", "precio", "duracion", "businessId", "categoryId", "updatedAt") 
                ...
                if (err) console.log("⚠️ Error en Cancha:", err.message);
            });
            ...
            db.run(`INSERT OR IGNORE INTO "_StaffServices" ("A", "B") 
                ...
                console.log("✅ Restauración completada con nombres legacy.");
                db.close();
            });
        });
    });
