...
async function main() {
    try {
        console.log('--- NEGOCIOS Y SUSCRIPCIONES ---');
        ...
        for (const n of negocios) {
            console.log(`Negocio: "${n.nombre}" (ID: ${n.id})`);
            if (n.Suscripcion) {
                const sub = n.Suscripcion;
                console.log(`  Suscripción ID: ${sub.id}`);
                console.log(`  Plan: "${sub.Plan?.name}" (ID: ${sub.Plan?.id})`);
                console.log(`  Estado: ${sub.estado}`);
                console.log(`  Locked Price: ${sub.lockedPrice}`);
                console.log(`  Custom Features: ${JSON.stringify(sub.customFeatures)}`);
                console.log(`  Plan Features: ${JSON.stringify(sub.Plan?.features)}`);
            } else {
