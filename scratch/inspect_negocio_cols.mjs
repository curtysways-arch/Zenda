...
conn.on("ready", () => {
  console.log("Conectado al VPS. Obteniendo columnas de Negocio...");
...
async function run() {
    ...
    const cols = await client.execute("PRAGMA table_info(Negocio)");
    console.log("Columnas en Negocio:");
    console.table(cols.rows.map(r => ({ name: r.name, type: r.type, notnull: r.notnull })));
}
run().catch(console.error);
  `;

  conn.exec(`cat << 'EOF' > /opt/Zenda/get_cols.js\n${jsCode}\nEOF\ncd /opt/Zenda && node get_cols.js`, (err, stream) => {
    if (err) {
      console.error(err);
      conn.end();
      return;
