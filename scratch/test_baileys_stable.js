...
conn.on('ready', () => {
    console.log('📦 Instalando versión ESTABLE @whiskeysockets/baileys@6.7.18 en VPS...');
    conn.exec('cd /opt/Zenda/bot/whatsapp-bot && npm install @whiskeysockets/baileys@6.7.18', (err, stream) => {
        if (err) {
            console.error(err);
            conn.end();
            return;
