...
export default function TotemPantallaPage() {
  ...
  const fetchTotemToken = async () => {
    try {
      ...
      if (res.ok) {
        ...
        setExpiresIn(data.expiresInSeconds || 90);
        setCountdown(data.expiresInSeconds || 90);
      }
    } catch (e) {
  ...
  // Cuenta regresiva del token y auto-refresco a los 0s
  useEffect(() => {
  ...
  const progressPercent = ((expiresIn - countdown) / expiresIn) * 100;
      ...
      <main className="flex-1 flex flex-col items-center justify-center my-6 z-10">
        <div className="relative group">
          ...
          <div className="relative bg-slate-900 border-2 border-slate-800 rounded-[38px] p-8 md:p-12 shadow-2xl flex flex-col items-center text-center max-w-md w-full">
            {/* Tag de seguridad */}
            <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-xs font-black uppercase tracking-widest mb-6">
              <ShieldCheck className="w-4 h-4" />
              <span>Acceso Seguro Dinámico</span>
            </div>
            ...
            {/* Barra de progreso de rotación de token */}
