...
export default function AdminDespachoPage() {
  ...
  const getStatusBadge = (estado: string) => {
    switch (estado) {
      case 'EN_PREPARACION':
      case 'PREPARANDO':
        return <span className="px-2.5 py-1 bg-amber-100 text-amber-900 border border-amber-300 text-[10px] font-black rounded-lg uppercase tracking-wider">En Preparación</span>;
      case 'RECIBIDO':
      case 'PENDIENTE':
      case 'WAITING_CONFIRMATION':
        return <span className="px-2.5 py-1 bg-blue-100 text-blue-900 border border-blue-300 text-[10px] font-black rounded-lg uppercase tracking-wider">Recibido</span>;
      case 'LISTO':
      case 'LISTA':
        return <span className="px-2.5 py-1 bg-emerald-100 text-emerald-900 border border-emerald-300 text-[10px] font-black rounded-lg uppercase tracking-wider">Listo</span>;
      ...
      case 'EN_CAMINO':
      case 'EN_RUTA':
        return <span className="px-2.5 py-1 bg-sky-100 text-sky-800 text-[10px] font-black rounded-lg uppercase tracking-wider flex items-center gap-1"><Truck className="size-3 animate-pulse" /> En Ruta</span>;
      case 'ENTREGADO':
      case 'ENTREGADO_MESA':
      case 'RETIRADO':
        return <span className="px-2.5 py-1 bg-slate-100 text-slate-800 border border-slate-300 text-[10px] font-black rounded-lg uppercase tracking-wider">Entregado</span>;
      default:
