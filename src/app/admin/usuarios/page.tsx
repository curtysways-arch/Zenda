    ...
    const fetchUsuarios = async (q = "") => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                // Si venimos por parámetro, redirigir a la pantalla completa de nuevo / editar
                if (phoneParam && !isByParam) {
                    setIsByParam(true);
                    const exactUser = data.find((u: Usuario) => u.phone === phoneParam);
                    if (exactUser) {
                        router.push(`/admin/usuarios/nuevo?id=${exactUser.id}`);
                    } else {
                        const nameParam = searchParams.get("name") || "";
                        router.push(`/admin/usuarios/nuevo?phone=${phoneParam}&name=${encodeURIComponent(nameParam)}`);
                    }
                }
            }
        } catch (error) {
        ...
        <div className="p-4 md:p-8 space-y-8 animate-in fade-in duration-500">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <Link 
                    ...
                    className="inline-flex items-center gap-2 text-white font-black px-6 py-4 rounded-[2rem] shadow-xl transition-all active:scale-95 uppercase tracking-widest text-xs"
            ...
            <div className="relative group flex items-center">
                <Search className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-[var(--primary-color)] transition-colors pointer-events-none z-10" size={20} />
                <input 
                    type="text"
                        ...
                        <div key={user.id} className="bg-white rounded-[3rem] border border-slate-100 p-8 space-y-6 shadow-sm hover:shadow-xl transition-all group relative overflow-hidden">
                            <div className="absolute top-0 right-0 p-4">
                                <div className="flex gap-2">
                                    <Link href={`/admin/usuarios/nuevo?id=${user.id}`} className="p-3 bg-slate-50 text-slate-400 rounded-2xl transition-all inline-flex items-center justify-center"
