...
export async function GET(
    ...
    try {
        ...
        const formatted = {
            ...course,
            schedules: (course.CourseSchedule || []).map((s: any) => ({
                ...s,
                court: s.Service
            })),
            _count: {
...
export async function PATCH(
    ...
    try {
        ...
        const { name, description, imageUrl, min_age, max_age, coach, price, payment_type, capacity, status, start_date, end_date, content, instructor_id } = body;
...
export async function DELETE(
    ...
    try {
        ...
        if ((existing._count?.CourseEnrollment || 0) > 0) {
            return NextResponse.json({
