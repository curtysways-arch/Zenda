...
export async function PATCH(
    ...
    try {
        ...
        const current = await prisma.businessAdminTemplate.findUnique({
            where: { id }
        });

        if (!current) {
            return NextResponse.json({ error: 'Plantilla de admin no encontrada' }, { status: 404 });
        }
        ...
        const updated = await prisma.businessAdminTemplate.update({
            where: { id },
            data: {
                ...(body.name !== undefined && { name: body.name.trim() }),
                ...(body.component !== undefined && { component: body.component.trim() }),
                ...(body.layoutType !== undefined && { layoutType: body.layoutType.trim() }),
        ...
        console.error('Error al actualizar plantilla de admin:', error);
...
export async function DELETE(
    ...
    try {
        ...
        const { id } = await params;
        await prisma.businessAdminTemplate.delete({
            where: { id }
        });

        return NextResponse.json({ success: true, message: 'Plantilla eliminada correctamente' });
    } catch (error: any) {
        console.error('Error al eliminar plantilla de admin:', error);
