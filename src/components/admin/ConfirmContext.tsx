...
export function ConfirmProvider({ children, primaryColor = '#0ea5e9' }: { children: React.ReactNode; primaryColor?: string }) {
    ...
    return (
        <ConfirmContext.Provider value={{ confirm }}>
            ...
            {state?.isOpen && (
                <div 
                    className="fixed inset-0 z-[999999] overflow-y-auto bg-black/40 backdrop-blur-md flex items-start sm:items-center justify-center p-4 animate-in fade-in duration-300"
                    style={{ '--primary-color': primaryColor } as any}
                    onClick={handleCancel}
                >
                    {/* Tarjeta del modal con layout natural (mt-12 en móviles para evitar que el teclado lo empuje) */}
                    <div 
                        className={clsx(
                            "w-[95%] max-w-sm sm:max-w-md bg-white dark:bg-slate-900 border border-slate-100 dark:border-white/5 rounded-[2.5rem] p-5 sm:p-8 shadow-2xl relative overflow-hidden text-center mt-12 sm:my-auto",
